...
if [[ "$#" -lt 2 ]]; then
  echo "Usage: sbatch slum_scripts/submit_paper_chemical_pathA_lomo.sh <tau> <heldout_mod> [encoder=linear] [seed=1] [alpha_pwm=2.0] [alpha_meta=2.0] [sigma=1.5]"
  exit 2
fi
...
SAVE_SUFFIX="${SAVE_SUFFIX}_ap${ALPHA_PWM}_am${ALPHA_META}_s${SIGMA}"
...
python Scripts/paper_multirm.py train_chemical_pathA_lomo \
  --data_path Data/MultiRM_data.h5 \
  --embedding_path Embeddings/embeddings_12RM.pkl \
  --modifications_path Data/modifications.csv \
  --save_dir "Results/paper_aligned/chemical_pathA_tau${TAU}_${ENC}${SAVE_SUFFIX}_lomo" \
