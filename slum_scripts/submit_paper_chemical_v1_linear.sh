...
if [[ "$#" -ne 1 ]]; then
  echo "Usage: sbatch slum_scripts/submit_paper_chemical_v1_linear.sh <bilinear|lowrank|hypernetwork>"
  exit 2
fi
...
python Scripts/paper_multirm.py train_chemical_v1 \
  ...
  --save_dir "Results/paper_aligned/chemical_v1_${SCORER_TYPE}_linenc" \
