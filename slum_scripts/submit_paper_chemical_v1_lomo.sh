...
#SBATCH -J multirm-chemical-v1-lomo
#SBATCH -o %x.%j.out
#SBATCH -e %x.%j.err
#SBATCH --time=2:00:00
#SBATCH --gres=gpu:1
...
if [[ "$#" -ne 2 ]]; then
  echo "Usage: sbatch slum_scripts/submit_paper_chemical_v1_lomo.sh <bilinear|lowrank|hypernetwork> <Am|Cm|Gm|Um|m1A|m5C|m5U|m6A|m6Am|m7G|Psi|I>"
  exit 2
fi
...
echo "scorer_type=${SCORER_TYPE} heldout_mod=${HELDOUT_MOD}"
...
python Scripts/paper_multirm.py train_chemical_v1_lomo \
  ...
  --save_dir "Results/paper_aligned/chemical_v1_${SCORER_TYPE}_lomo" \
