...
#SBATCH -J multirm-v1-frozenenc-lomo
#SBATCH -o %x.%j.out
#SBATCH -e %x.%j.err
#SBATCH --time=2:00:00
#SBATCH --gres=gpu:1
...
if [[ "$#" -ne 2 ]]; then
  echo "Usage: sbatch slum_scripts/submit_paper_chemical_v1_frozen_lomo.sh <bilinear|lowrank|hypernetwork> <heldout_mod>"
  exit 2
fi
...
python Scripts/paper_multirm.py train_chemical_v1_lomo \
  ...
  --save_dir "Results/paper_aligned/chemical_v1_${SCORER_TYPE}_frozenenc_lomo" \
  --cache_dir Results/paper_aligned/cache \
  --scorer_type "${SCORER_TYPE}" \
  --chemical_encoder_type frozen_linear \
  --num_heads 8 \
